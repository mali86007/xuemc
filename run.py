...
from DB import orm
from web.Logic import logic, restful
...
def pre_put_single_account(instance_id=None, data=None, **kw):
    ...
    if data.has_key(restful.ITEM_CHECKCODE):
        ...
        if account.checkcode == data.get(restful.ITEM_CHECKCODE) and dtNow < dtValidTime:   # 如验证一致，且在15分内
            account.flag_telephone = 1              # 用户电话标识
            orm.db.session.commit()
...
def post_post_account(result=None, **kw):
    ...
    新建用户后，向用户手机发送验证码（为用户的POST请求而增加）
    """
    if result and result.has_key(restful.ITEM_ID):
        ...
        Util.SendSMSByZA(account.telephone, message)                    # 实际发送验证码
    return result
    pass
